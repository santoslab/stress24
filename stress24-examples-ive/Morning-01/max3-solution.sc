...
@pure def max3(x:Z, y:Z, z:Z): Z = {
  Contract(
    Ensures(Res[Z] == x | Res[Z] == y | Res[Z] == z,
           x <= Res[Z] & y <= Res[Z] & z <= Res[Z])
  )
...
// ToDo: Need to provide a reference to universal quantification syntax
Deduce(
   |- (All{(q: Z) => max3(q,q,q) == q})
)
